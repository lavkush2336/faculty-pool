<?php
// teacher-password-reset.php (Step 2: OTP and Password Reset)
session_start();
...
// 2. DEFINE YOUR EMAIL FUNCTION (Included for file independence)
...
// SECURITY CHECK: Redirect if no email session is set (direct access)
if (!isset($_SESSION['otp_email']) || !isset($_SESSION['otp'])) {
    header('Location: teacher-password-email.php');
    exit;
}
...
if (time() - ($_SESSION['otp_time'] ?? 0) > 600) { 
     ...
     header('Location: teacher-password-email.php?status=expired');
     exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_password'])) {
    ...
    elseif (time() - ($_SESSION['otp_time'] ?? 0) > 600) {
        ...
        try {
            ...
            // ⭐️ IMPORTANT: Using 'password1' column for password update, as requested.
            $stmt = $pdo->prepare("UPDATE Faculty SET password1 = :password WHERE email = :email");
            $stmt->execute([
            ...
            header('Location: teacher-login.php?status=pwreset');
            exit;
  ...
  <div class="forgot-container">
    <div class="forgot-card">
      ...
      <div class="login-links">
        ...
        <a href="teacher-password-email.php">
